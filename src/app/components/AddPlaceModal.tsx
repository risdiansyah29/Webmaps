...
export function AddPlaceModal({ open, onClose, onAdd, isDark }: Props) {
  ...
  const inputCls = `w-full px-3 py-2 rounded-lg border text-sm focus:outline-none focus:ring-2 ${
    "bg-[var(--app-surface)] border-[var(--app-border)] text-[var(--app-text)] placeholder:text-[var(--app-text-muted)] focus:ring-[color:var(--app-accent)]/20 focus:border-[var(--app-accent)]"
  }`;
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 bg-[var(--app-overlay)] z-[1100] flex items-end md:items-center justify-center p-0 md:p-6"
          onClick={onClose}
        >
          <motion.div
            ...
            className="w-full md:max-w-lg rounded-t-3xl md:rounded-3xl overflow-hidden shadow-2xl bg-[var(--app-bg)]"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between p-4 border-b border-[var(--app-border)]">
              <h3 className="text-[var(--app-text)]">Tambah tempat baru</h3>
              <button onClick={onClose} className="w-9 h-9 rounded-full hover:bg-[var(--app-surface-2)] flex items-center justify-center">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={submit} className="p-4 space-y-3 max-h-[70vh] overflow-y-auto">
              ...
              <p className="text-xs text-[var(--app-text-muted)]">Tips: klik kanan di Google Maps untuk menyalin koordinat.</p>
              {error && <p className="text-sm text-[var(--app-danger-text)]">{error}</p>}
              <div className="flex gap-2 pt-2">
                <button
                  type="button"
                  onClick={onClose}
                  className="flex-1 py-2.5 rounded-full border text-sm border-[var(--app-border)] bg-[var(--app-surface)] text-[var(--app-text)] hover:opacity-95"
                >
                  Batal
                </button>
